<template>
  <div class="pay-main">
    <el-button type="danger" size="mini">测试</el-button>
    <div class="pay-container">
      <div class="checkout-tit">
        ...
        <div class="paymark">
          ...
          >
          <span class="fr"
            ><em class="lead">应付金额：</em><em class="orange money">￥{{ payInfo.totalFee }}.00元</em></span
          >
        </div>
      </div>
...
export default {
  ...
  methods: {
    // 获取订单号发送给服务器
    async getOrderPay() {
      const res = await this.$API.reqOrderPay(this.orderId)
      console.log(res)
      if (res.code === 200) {
    ...
    async open() {
      ...
      if (!this.timer) {
        this.timer = setInterval(async () => {
          ...
          if (res.code === 200) {
            clearInterval(this.timer) // 清除定时器
            this.timer = null // 重置为空
            this.code = res.code // 保存支付状态
            this.$msgbox.close()
            this.$router.push('/paysuccess')
          }
        }, 1000)
